...
| sso\_enabled | Whether the cluster deploys dex -- pass the cluster module's sso.enabled. Creates the dex-google-* containers: the<br/>Google OAuth app dex's google connector signs users in with (an OAuth client cannot be terraformed -- create it in<br/>the console and add the versions out of band) plus the Workspace admin email the directory reads impersonate. | `bool` | `false` | no |
