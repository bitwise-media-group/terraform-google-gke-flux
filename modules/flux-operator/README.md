...
| signed\_identity | Cosign keyless identity (Go regexps over the Fulcio certificate) enforced on the generated flux-system<br/>OCIRepository, so an unsigned or tampered manifests artifact is never applied. | <pre>object({<br/>    issuer            = string<br/>    manifests_subject = string<br/>  })</pre> | n/a | yes |
...
| grant\_registry\_read | Grant the flux principals artifactregistry.reader on the cluster's project. On only when the platform registry is<br/>co-located; a central registry grants them via the artifact-store module's reader\_members instead. | `bool` | `true` | no |
